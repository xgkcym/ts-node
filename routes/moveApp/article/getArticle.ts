...
module.exports = async (req: any, res: Response) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        const userslike_articleModelIndex = userslike_articleModel.findIndex(v=>v.uid === req.query.uid)
        ...
        const usersunlike_articleModelIndex = usersunlike_articleModel.findIndex(v=>v.uid === req.query.uid)
        ...
        const commentdata = await CommentModel.get(commentClass)
        let Comment1data = []
        for (let j = 0; j < commentdata.length; j++) {
            ...
            const userslike_commentModelIndex = userslike_commentModel.findIndex(v=>v.uid === req.query.uid)
            ...
            const usersunlike_commentModelIndex = usersunlike_commentModel.findIndex(v=>v.uid === req.query.uid)
            if(usersunlike_commentModelIndex == -1){
                Commentdata.isunlike = false
            }else{
                Commentdata.isunlike = true
            }
            const comment2Class = new Comment2Class({ article_id: commentdata[j].article_id })
           const comment2data =  await Comment2Model.get(comment2Class)
           let Commen2tdata = []
            for (let k = 0; k < comment2data.length; k++) {
                             const Commentdata: any = comment2data[k]
                            const userslike_commentClass = new Userslike_commentClass({ comment_id: commentdata[j].comment_id })
                            const userslike_commentModel = await Userslike_commentModel.get(userslike_commentClass)
                            Commentdata.likeNum = userslike_commentModel.length
                            const userslike_commentModelIndex = userslike_commentModel.findIndex(v=>v.uid === req.query.uid)
